    ...
    const previews = await Promise.all(
      rooms.map(async (room) => {
        ...
        return {
          ...
          name:
            room.type === "direct"
              ? room.participants.find((p) => p.user._id.toString() !== userId.toString())?.user.name
              : room.name,
...
export const markChatAsRead = async (req, res) => {
  try {
    ...
    // 1️⃣ Update all unread messages for this user
    ...
    // 2️⃣ Update ChatRoom's readReceipts array (optional, for analytics / group read)
      ...
      { _id: chatRoomId },
      {
        $push: {
          readReceipts: {
            $each: [
              {
                messageId: null, // or you can track latest message id
                userId,
                readAt: new Date()
              }
            ]
    ...
    // 3️⃣ Emit event via Socket.IO (so sender sees ✅✅ instantly)
    req.io.to(chatRoomId.toString()).emit("messages:read", {
      chatRoomId,
      readerId: userId,
    });
