...
const ChatWindow = ({ selectedChat, isGroup = false, currentUser, onToggleGroupInfo }) => {
  ...
  const getUserData = () => {
    try {
      ...
      if (!userId) {
        console.error('❌ No user ID available in getUserData');
  ...
  useEffect(() => {
    ...
    if (!userId) {
      console.log('⏳ Waiting for user ID before connecting socket');
      return;
    }
  ...
  const fetchMessages = useCallback(async () => {
    // ✅ FIXED: Check if selectedChat has proper _id
    if (!selectedChat?._id) {
      console.error('❌ Cannot fetch messages: selectedChat has no _id', selectedChat);
      return;
    }
  ...
  // ✅ FIXED: Enhanced typing handler with user check
  ...
  }, [socket, isConnected, selectedChat?._id, userId, isGroup, user, currentChatRoomId]);
  ...
  // ✅ FIXED: Check if we have proper chat data
        ...
        <div className="text-center space-y-6">
          ...
          <div>
            <h3 className="text-2xl font-bold bg-gradient-to-r from-indigo-600 to-purple-600 bg-clip-text text-transparent mb-2">
              {!selectedChat ? 'Start a Conversation' : 'Invalid Chat'}
            </h3>
            <p className="text-gray-500 dark:text-gray-400">
              {!selectedChat 
                ? 'Select a chat from the sidebar to begin messaging' 
                : 'The selected chat is not valid. Please select another chat.'}
            </p>
            {!userId && (
      ...
      <div className="h-20 flex-shrink-0 flex items-center gap-3 px-6 bg-gradient-to-r from-gray-800 to-gray-900 backdrop-blur-xl border-t border-purple-500/30 shadow-lg">
        ...
        <div className="flex-1 relative">
          <Input
            ...
            disabled={isSending || !isConnected || !userId}
            placeholder={!userId ? "Please login..." : !isConnected ? "Connecting..." : isSending ? "Sending..." : "Type your message..."}
        ...
        <motion.button
          ...
          disabled={!newMessage.trim() || !isConnected || isSending || !userId}
                    ...
                    <div className="flex items-center gap-4 mb-6">
                      ...
                      <div>
                        ...
                        <p className="text-gray-300">
                          Status: {onlineUsers.has(selectedChat._id) ? 'Online' : 'Offline'}
                        </p>
                      </div>
                    </div>
                  </div>
